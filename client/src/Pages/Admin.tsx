...
export default function Admin() {
  ...
  const fetchAppointments = async (date: any) => {
    try {
      const response = await fetch(`http://localhost:7000/appointments/date/${date}`)
      if (response.ok) {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this appointment?')) {
      try {
        const response = await fetch(`http://localhost:7000/appointments/${id}`, {
          method: 'DELETE',
        })
        if (response.ok) {
...
// }
//     try {
//       const response = await fetch(`http://localhost:7000/appointments/${selectedAppointment._id}`, {
//         method: 'PUT',
//         headers: {
